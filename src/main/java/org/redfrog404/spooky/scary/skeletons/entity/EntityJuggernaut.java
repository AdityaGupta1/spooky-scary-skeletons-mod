...
public class EntityJuggernaut extends EntityMob implements IBossDisplayData {

	@Override
	public IChatComponent getDisplayName() {
		super.getDisplayName();
		return new ChatComponentText("The Juggernaut");
	}
	...
	public int getTotalArmorValue() {
		return this.getHealth() <= this.getMaxHealth() / 2 ? 15 : 10;
	}
	...
	public void onKillEntity(EntityLivingBase entityLivingIn) {
		...
		if ((this.worldObj.getDifficulty() == EnumDifficulty.NORMAL || this.worldObj
			...
			EntityJuggernaut entityzombie = new EntityJuggernaut(this.worldObj);
	...
	public IEntityLivingData func_180482_a(DifficultyInstance p_180482_1_,
		...
		if (p_180482_2_1 == null) {
			p_180482_2_1 = new EntityJuggernaut.GroupData(
		...
		if (p_180482_2_1 instanceof EntityJuggernaut.GroupData) {
			EntityJuggernaut.GroupData groupdata = (EntityJuggernaut.GroupData) p_180482_2_1;
	...
	class AIGolemThrow extends EntityAIBase {
		...
		public void updateTask() {
			...
			if (entitylivingbase.getDistanceSqToEntity(this.juggernaut) < d0
					* d0
					&& this.juggernaut.canEntityBeSeen(entitylivingbase)) {
				World world = this.juggernaut.worldObj;
				++this.field_179471_a;

				if (this.field_179471_a == 10) {
					world.playAuxSFXAtEntity((EntityPlayer) null, 1007,
							new BlockPos(this.juggernaut), 0);
				}

				if (this.field_179471_a == 20) {
					double d1 = 4.0D;
					Vec3 vec3 = this.juggernaut.getLook(1.0F);
					double d2 = entitylivingbase.posX
							- (this.juggernaut.posX + vec3.xCoord * d1);
					double d3 = entitylivingbase.getEntityBoundingBox().minY
							+ (double) (entitylivingbase.height / 2.0F)
							- (0.5D + this.juggernaut.posY + (double) (this.juggernaut.height / 2.0F));
					double d4 = entitylivingbase.posZ
							- (this.juggernaut.posZ + vec3.zCoord * d1);
					world.playAuxSFXAtEntity((EntityPlayer) null, 1008,
							new BlockPos(this.juggernaut), 0);
					EntityIncineratorFireball fireball = new EntityIncineratorFireball(
							world, this.juggernaut, d2, d3, d4);
					fireball.explosionPower = this.juggernaut.getStrength();
					fireball.posX = this.juggernaut.posX + vec3.xCoord * d1;
					fireball.posY = this.juggernaut.posY
